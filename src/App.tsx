...
export const App: React.FC = () => {
  ...
  const [studentId, setStudentId] = useState<string>(() => getGuestStudentId());
  ...
  const joinPathMatch = window.location.pathname.match(/^\/join\/([^/]+)\/?$/i);
  const joinCode = joinPathMatch ? decodeURIComponent(joinPathMatch[1]) : '';
  const joinRequested = new URLSearchParams(window.location.search).has('join') || Boolean(joinCode);
  ...
  // Handle Joining Quiz
  const handleJoinQuiz = (quiz: Quiz, name: string, userAvatar: string, sessionId: string, joinedStudentId: string) => {
    setActiveQuiz(quiz);
    setActiveSessionId(sessionId);
    setStudentName(name || currentUser?.name || 'Player');
    setAvatar(userAvatar || currentUser?.avatar || '🚀');
    setStudentId(joinedStudentId || getGuestStudentId());
  ...
  const handleResetStudent = () => {
    ...
    setStudentState('join');
    setStudentId(getGuestStudentId());
  };
  ...
  return (
    <div className={`min-h-screen ${isDarkMode ? 'bg-slate-950 text-white' : 'bg-slate-100 text-slate-900'} transition-colors duration-200 flex flex-col font-sans selection:bg-purple-500 selection:text-white`}>
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {joinRequested && !activeQuiz ? (
          <JoinQuizModal onJoinSuccess={handleJoinQuiz} initialCode={joinCode} />
