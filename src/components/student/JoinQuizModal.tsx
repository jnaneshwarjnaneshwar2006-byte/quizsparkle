...
interface JoinQuizModalProps {
  onJoinSuccess: (quiz: Quiz, studentName: string, avatar: string, sessionId: string, studentId: string) => void;
  initialCode?: string;
}

const AVATARS = ['🚀', '⚡', '🧠', '🔥', '🎯', '🦁', '🦉', '🐱', '🐼', '🌈', '🍀', '🎸'];

const getGuestStudentId = (): string => {
  const storageKey = 'quizspark-guest-student-id';
  const existing = localStorage.getItem(storageKey);
  if (existing && /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(existing)) return existing;
  const secureRandomUuid = globalThis.crypto?.randomUUID;
  const created = typeof secureRandomUuid === 'function'
    ? secureRandomUuid.call(globalThis.crypto)
...
export const JoinQuizModal: React.FC<JoinQuizModalProps> = ({ onJoinSuccess, initialCode = '' }) => {
  ...
  const handleVerifyCode = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const session = await quizService.getWaitingLiveQuizSession(quiz.id);
      if (!session) {
        setErrorMsg('This quiz does not have an active live session yet. Ask the teacher to start it.');
        setIsJoining(false);
        return;
      }
      console.debug('[LIVE STUDENT] quiz code resolved to session', { code, quizId: quiz.id, sessionId: session.id });
      setIsJoining(false);
      setVerifiedQuiz(quiz);
      setSessionId(session.id);
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      console.error('[LIVE STUDENT] session lookup failed', { code, error });
      setErrorMsg(
        `Failed to connect to quiz session: ${message}`
      );
      setIsJoining(false);
    }
  };
  ...
  const handleJoin = async (e: React.FormEvent) => {
    ...
    if (!selectedAvatar) {
      setErrorMsg('Choose an emoji to join.');
      return;
    }
    if (!sessionId) {
      setErrorMsg('Live quiz session is no longer available. Check the code again.');
      return;
    }
    const studentId = getGuestStudentId();
    setIsJoining(true);
    try {
      await quizService.joinLiveQuiz(sessionId, verifiedQuiz.id, studentName.trim(), selectedAvatar, studentId);
      console.debug('[LIVE STUDENT] student registered in session', {
        quizId: verifiedQuiz.id,
        sessionId,
        studentId
      });
      onJoinSuccess(verifiedQuiz, studentName.trim(), selectedAvatar, sessionId, studentId);
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      console.error('[LIVE STUDENT] student registration failed', {
        quizId: verifiedQuiz.id,
        sessionId,
        studentId,
        error
      });
      setErrorMsg(`Could not join the live quiz: ${message}`);
    } finally {
      setIsJoining(false);
    }
  };
          ...
          {!verifiedQuiz ? (
            <form onSubmit={handleVerifyCode} className="space-y-4 bg-slate-950/80 p-6 rounded-3xl border border-slate-800 shadow-2xl backdrop-blur-md">
              ...
              <form onSubmit={handleJoin} className="space-y-5 bg-slate-950/80 p-6 rounded-3xl border border-emerald-500/30 shadow-xl text-left animate-fade-in">
                ...
                <button type="submit" disabled={isJoining} className="w-full py-4 rounded-2xl bg-gradient-to-r from-emerald-400 to-cyan-400 hover:from-emerald-300 hover:to-cyan-300 text-slate-950 font-black text-lg flex items-center justify-center gap-2 disabled:opacity-60"><Sparkles className="w-5 h-5" /> {isJoining ? 'Joining...' : 'Join the earth'}</button>
              </form>
            )}
