...
export const GameLobby: React.FC<GameLobbyProps> = ({
  ...
  const [players, setPlayers] = useState([{ name: studentName, avatar }]);

  useEffect(() => {
    const lobbyKey = `quiz-lobby-${quiz.id}`;
    const syncPlayers = () => {
      try {
        const stored = JSON.parse(localStorage.getItem(lobbyKey) || '[]') as { name: string; avatar: string }[];
        const nextPlayers = stored.some((player) => player.name === studentName)
          ? stored
          : [...stored, { name: studentName, avatar }];
        localStorage.setItem(lobbyKey, JSON.stringify(nextPlayers));
        setPlayers(nextPlayers);
      } catch {
        setPlayers([{ name: studentName, avatar }]);
      }
    };
    syncPlayers();
    const refresh = window.setInterval(syncPlayers, 1000);
    return () => window.clearInterval(refresh);
  }, [avatar, quiz.id, studentName]);

  useEffect(() => {
    const startKey = `quiz-start-${quiz.id}`;
    const checkStart = () => {
      if (localStorage.getItem(startKey)) onStartGame();
    };
    checkStart();
        ...
        <div className="space-y-3 pt-4 border-t border-slate-800/80">
          ...
          <div className="relative mx-auto w-64 h-64 rounded-full bg-gradient-to-br from-cyan-400 via-emerald-500 to-blue-900 border-8 border-cyan-200/20 shadow-[0_0_70px_rgba(34,211,238,0.25)] overflow-visible">
            ...
            {players.map((player, index) => ({ ...player, pos: ['left-2 top-14', 'right-0 top-8', 'right-1 bottom-12', 'left-5 bottom-5', 'left-0 top-28'][index % 5] })).map((player) => (
              <div key={`${player.name}-${player.avatar}`} className={`absolute ${player.pos} z-10 group`} title={player.name}>
          ...
          <div className="flex flex-wrap items-center justify-center gap-2">
            {players.map((player, index) => <span key={`${player.name}-${index}`} className={`px-3 py-1.5 rounded-full text-xs font-bold ${player.name === studentName ? 'bg-emerald-400/20 text-emerald-300 border border-emerald-400/40' : 'bg-slate-900 text-slate-300 border border-slate-800'}`}>{player.avatar} {player.name}{player.name === studentName ? ' (you)' : ''}</span>)}
          </div>
        </div>

        {/* LAUNCH BUTTON */}
        <div className="pt-4">
          <div className="w-full max-w-md mx-auto py-4 rounded-2xl bg-slate-800 border border-slate-700 text-slate-300 font-black text-lg flex items-center justify-center gap-3"><span className="animate-pulse">⏳</span> Waiting for teacher to start</div>
        </div>
