...
import type { Quiz, LobbyPlayer, LiveQuizParticipant, LiveQuizSession } from '../../types/quiz';
...
export const TeacherLobby: React.FC<TeacherLobbyProps> = ({ quiz, session, onBack }) => {
  ...
  useEffect(() => {
    ...
    const syncLobby = async () => {
      try {
        const participants = await quizService.getLiveQuizParticipants(session.id);
        console.debug('[LIVE TEACHER] participants fetched', { quizId: quiz.id, sessionId: session.id, participants });
        if (active) {
        ...
        console.error('[LIVE TEACHER] participant fetch failed', { quizId: quiz.id, sessionId: session.id, error });
      }
    };

    console.debug('[LIVE TEACHER] opening session', { quizId: quiz.id, quizCode: quiz.code, sessionId: session.id });
    void syncLobby();
    cleanup = quizService.subscribeToLiveQuizParticipants(session.id, (participant: LiveQuizParticipant) => {
      if (!active) return;
      setPlayers(previous => {
        if (previous.some(player => player.id === participant.id)) return previous;
        return [...previous, {
          id: participant.id,
          quiz_id: participant.quiz_id,
          student_name: participant.student_name,
    ...
    }, status => console.debug('[LIVE TEACHER] realtime status', { sessionId: session.id, status }));
    return () => { active = false; cleanup(); };
  }, [quiz.id, quiz.code, session.id]);

  const startQuiz = () => {
    soundFx.playClick();
    void quizService.updateLiveQuizSessionStatus(session.id, 'live')
      .then(() => setHasStarted(true))
      .catch(error => console.error('[LIVE TEACHER] failed to start session', { sessionId: session.id, error }));
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-[0.8fr_1.2fr] gap-6 items-center">
            <div className="relative mx-auto w-56 h-56 rounded-full bg-gradient-to-br from-cyan-400 via-emerald-500 to-blue-900 border-8 border-cyan-200/20 shadow-[0_0_70px_rgba(34,211,238,0.25)]">
              ...
              {players.map((player, index) => (
                <span key={`${player.student_name}-${index}`} title={player.student_name} className={`absolute flex items-center justify-center w-11 h-11 rounded-full bg-slate-950 border-2 border-emerald-300 text-2xl shadow-lg ${['-left-2 top-10', '-right-2 top-16', 'left-4 -bottom-2', 'right-4 -bottom-2'][index % 4]}`}>
                  {player.avatar}
                </span>
              ))}
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="min-h-40 max-h-64 overflow-y-auto rounded-2xl bg-slate-950/70 border border-slate-800 p-3 space-y-2">
                {players.length === 0 ? <p className="h-32 flex items-center justify-center text-sm text-slate-500">Waiting for students to join...</p> : players.map((player, index) => <div key={`${player.student_name}-${index}`} className="flex items-center gap-3 rounded-xl bg-slate-900 border border-slate-800 px-3 py-2"><span className="text-2xl">{player.avatar}</span><span className="text-white font-bold text-sm">{player.student_name}</span><span className="ml-auto text-[10px] uppercase tracking-wider text-emerald-400 font-black">Ready</span></div>)}
              </div>
